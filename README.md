...
- 일반적인 변수는 값이 바뀌지만
- Generic은 값이 아니고 `변수의 종류를 변경`함

## 1. 문제 상황 살펴보기

- 매개변수의 `종류만 다르고` 기능은 같을 때
- 아래 코드를 자주 보게 됨
...
showArgA(0); // 실행 중 오류 발생
showArgA("안녕"); // 정상작동
```

## 2. 문제 상황을 Generic으로 해결해보기
...
- 아래의 코드는 아직도 오류로 처리됨
- 하지만 실행 중 오류가 아니고 코딩 중 오류를 표현함

```ts
// T를 타입변수로 생각하면 좋음
function showArgGeneric<T>(a: T): void {
  console.log(a);
  // 아래의 문제는 타입 좁히기로 해결이 가능함
  // 타입 가드라고 함
  console.log(a.length);
}
...
- 우선 해결코드를 진행하고 다시 문법을 보자

```ts
function showArgGeneric<T>(a: T): void {
  console.log(a);
  // console.log(a.length); // 아래에서 if로 오류 처리 진행
...
- any로 해결함. (데이터 체크를 포기함)
- 혹시 지금은 에러가 없는데 나중에 에러가 발생하지 않을까?하는 불안감 발생
...
- `제네릭을 사용`하면 코딩 중에 오류발견 쉽고 서비스 중에도 대응 수월함
...
const result2: boolean = findeItem([12, 20, 30], 20);
...
- Gemeric으로 해결해 봄 (코딩 중 오류, 실행 중 오류 파악 용이)
...
class TodoStorage {
  ...
  // 메소드만으로 즉, 검증된 과정으로만 내부 item 배열 접근
