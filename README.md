...
- constructor 메서드: 속성의 초기값을 세팅해줄 수 있음
...
const ins2 = new Robot("팬여러분");
console.log(ins2); // Robot { who: '팬여러분' }
...
const ins2: Robot = new Robot("팬여러분");
console.log(ins2); // Robot { who: '팬여러분' }
...
- protected 속성 접근 제어자 (`상속` 받은 대상만 접근 가능)
...
class Cat extends Animal {
  ...
  show() {
    ...
    this.nose; // private이라서 아무도 접근 못함
    this.lips; // protected는 자식이라서 허용됨
  }
}
// 인스턴스 객체 생성
const cat = new Cat;
cat.eye; // public이라서 마음대로 접근 가능
cat.nose; // Error, private라서 아무도 못 접근 불가
cat.lips; // Error, protected라서 아무도 접근 불가
```
