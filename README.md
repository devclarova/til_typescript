...
// 오름차순으로 정리하기 위해서 추가 작업 필요
const numArr = [1, 12, 3, 19, 7, 5];
numArr.sort((a, b) => a - b); // 1, 3, 5, 7, 12, 19
// 원본 배열이 훼손 ! 주의사항

// 내림차순으로 정리하기 위해서 추가 작업 필요
const numArr = [1, 12, 3, 19, 7, 5];
...
- `원본 배열`과 `본사본 배열`의 `요소를 비교`해서 다르면 화면 새로 그림(`리렌더링/Re-rendering`)
...
```js
const 원본배열 = [1, 2, 3, 4];
const 복사본 = 원본배열.map(function (요소, 인데스, 원본배열) {
  return 요소;
}); // [1, 2, 3, 4]
// html 예제
const 복사본 = 원본배열.map(function (요소, 인데스, 원본배열) {
...
```js
const numArr = [1, 2, 3];
const result = numArr.includes(3); // ture
```
