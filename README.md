...
// 타입스크립트는 실제 값의 종류를 따지는 것이 아니라
// 데이터를 할당할 때(대입할 때) 데이터 종류만 비교함
let testNum: number = anyVar;
```

- `any 타입은 어떤 다른 타입에도 값을 담을 수 있음`
- `any 타입은 어떤 다른 타입에도 값을 할당할 수 있음`
- `any 타입은 어떤 다른 타입에도 값을 대입할 수 있음`

### 10.2. unknown 타입

- 데이터 종류를 모르겠다 (마치 any처럼)
- 값으로 전달하지 못함(any와는 다르게)
...
1. 모두 소문자로 작성
2. 단어 사이에 -를 붙임
3. 케밥 케이스는 코딩에 사용 못함(css 클래스명으로 활용)
let user-name;
```
