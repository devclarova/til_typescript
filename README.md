...
- primitive 데이터(기본형 데이터)
  ...
  - any, unknown, never
- 기본형 데이터가 너무 많다면 묶어서 관리
  - 이를 참조 타입 즉, Reference 타입이라고 함
  - Reference 타입을 객체라고 함
...
- 사용자 정보가 너무 많음. 그래서 관리(변수명 짓기) 등이 어려움
...
const userInfoGenric: Array<number | boolean | string> = ["kr", true, 53];
```
...
- `원시데이터를 묶어서 관리`함
- 데이터의 각 요소의 `이름 즉, 속성(Property)를 정해서 관리`함
- 아래 문장을 `객체 리터럴`이라고 하며 매우 중요함
