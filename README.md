...
- `javaScript 데이터 종류(Data Type)을 정의하는 언어`
- 타입스크립트 파일은 js가 아니라서 웹브라우저 및 Node.js에서 실행 불가
- 타입스크립트로 작성하면 자동으로 js 파일로 번역
- 그 결과로 만들어진 js를 실행

## 3. TypeScript 특징

- 코딩 중에 오류를 발견하기 쉬움 (디버깅 수월)
- js는 코딩 중에 오류를 발견하기 어려움 (심각한 문제 발생)

## 4. TypeScript 프로젝트 세팅

- Node를 설치하고 나면 자동으로 npm이 설치
...
- ts 생성 > 컴파일러 실행 > js 생성
- 대표적 js 생성 시 옵션들(https://www.typescriptlang.org/tsconfig/)
...
- 모두 TypeScript로 작성 권장
- tsconfig.json이 상당히 중요
