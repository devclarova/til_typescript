...
const jennieMom: Parent = new Parent("이아무개");
// child는 Parent를 확장(extends)
const jennie: Child = new Child(28, "김아무개");

// 상속받은 자식은 값으로서 부모에게 할당할 수 있다.
const mom: Parent = jennie;

// 상속받은 자식에게 부모는 값으로 할당할 수 없다.
const son: Child = jennieMom; // Error
```
...
// 아래는 클래스에 속성 및 메서드가 한개도 없으므로 가능함
...
const 옹이: Cat = 동물; // age가 없어서 오류 발생
```
...
// Cat은 필수 속성인 age가 필요하지만 Animal에는 없다.
const 옹이: Cat = 동물; // age가 없어서 오류 발생
// Dog는 필수 속성인 breez가 필요하지만 Animal에는 없다.
const 멍이: Dog = 동물; // breez가 없어서 오류 발생
```
