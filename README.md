...
- 추후 진행 시에는 type에 대한 정의를 먼저 고민해보자
...
- `{속성명:속성값, 속성명:속성값}`처럼 여러 개를 묶어둔 상태
...
### 2.3. `확장` 가능 (기존 type을 확장해서 또 다른 type 작성)
...
- 입력값, 리턴값 모두 타입을 정의할 수 있음
- `type 타입명: (매개변수:타입) => 리턴값타입`

```ts
type Add = (a: numberm, b: number) => number;
```
...
const hollo2: SayHello = (msg: string) => {
  console.log(msg);
};
```
