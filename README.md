...
- 옵셔널을 사용하면 매개변수가 필수값이 아니다.
- 선택적으로 활용할 수 있다.
- 옵셔널을 활용하면 undefined를 허용하겠다.
...
const result = showName("제니");
const result2 = showName("로제", 20);
```

## 함수의 매개변수 기본값(default Value)

- 사용자가 만약 값을 넣지 않으면 기본값으로 처리
...
const result = showName("제니", "미국");
const result2 = showName("로제");
...
- 어떤 변수 또는 어떤 반환값이 `특정 타입인지 확인하는 용도의 함수`

```ts
// 어떤 재교가 들어왔을 때 number인지 확인하는 함수
...
- 타입오류 발생 시 조치사항
- `any 변경` > `Narrowing(타입 좁히기)` ? `as로 강제타입` > `is함수`
