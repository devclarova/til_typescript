...
// falshy한 값이므로 false로 판단
// false, null, undefined, ""(공백이 들어가면 참), 0, NaN
if (result) {
  console.log("결과 참");
} else {
  console.log("결과 거짓");
...
- switch문의 값은 직접 원시값으로 작성 시 오류발생이 가능
- TypeScript에 있는 `enum`을 사용하길 권장
...
for (let i: nember = 0; i < total; i += 1) {
  if (i === 5) {
    break; // fot 구문을 벗어남
  }
  // 실행코드
...
- `일단 한번은 실행`하고 `조건이 참`이면서 `반복 횟수를 모를 때` 사용

```ts
// 아래 코드는 한번도 실행되지 않음. 조건이 거짓이므로
let count: number = 5;
while (count < 5) {
  // 실행
  count = count + 1; // 반드시 조건을 거짓으로 만들도록 코드 필요
